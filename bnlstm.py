...
Adapted from: https://github.com/jihunchoi/recurrent-batch-normalization-pytorch
Credit to: Jihun Choi
"""
import torch
    ...
    def reset_parameters(self):
        ...
        with torch.no_grad():
            ...
            if self.use_bias:
                ...
                torch.split(self.bias.data, self.hidden_size)[0].fill_(1) #XXX except fgate
...
class BNLSTMCell(nn.Module):
    ...
    def reset_parameters(self):
        ...
        with torch.no_grad():
            ...
            # The bias is just set to zero vectors.
            init.constant_(self.bias.data, val=0)
            #XXX Nope, setting forget gate biases to 1 just in case
            torch.split(self.bias.data, self.hidden_size)[0].fill_(1)
            ...
            #XXX Modified here to init forget gate BN biases to 1s for mem fixing
            torch.split(self.bn_ih.bias.data, self.hidden_size)[0].fill_(1)
            torch.split(self.bn_hh.bias.data, self.hidden_size)[0].fill_(1)
    ...
    def forward(self, input_, hx, time):
        ...
        f, i, o, g = torch.split(bn_wh + bn_wi + bias_batch,
                                 split_size_or_sections=self.hidden_size, dim=1)
        c_1 = torch.sigmoid(f)*c_0 + torch.sigmoid(i)*torch.tanh(g)
        ot = self.bn_c(c_1, time=time)
        h_1 = torch.sigmoid(o) * torch.tanh(ot)
        return h_1, c_1#, f, i, o, g, ot
